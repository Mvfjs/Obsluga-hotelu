...
namespace WinFormsApp1
{
    public partial class GuestLogin : Form
    {
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            string myConnectionString = "Server=127.0.0.1;Port=3306;Uid=root;Pwd=;Database=the garden hotel2;";
            using (MySqlConnection conn = new MySqlConnection(myConnectionString))
            {
                try
                {
                    ...
                    // Zapytanie SQL do sprawdzenia danych logowania bierzemy reservationcode i roomid do logowania
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                Guest guest = new Guest(reservationCode, roomID);
                                this.Hide();
                                guest.Show();
                            }
                            else
