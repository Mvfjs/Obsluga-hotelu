    ...
    public partial class reservation : ValidationForm
    {
        ...
        private void LoadRoomAvailability()
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    MessageBox.Show($"Błąd podczas ładowania dostępności: {ex.Message}", "Błąd", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void buttonConfirm_Click(object sender, EventArgs e)
        {
            ...
            var (isValid, firstName, lastName) = ValidateAndSplitFullName(fullName);
            if (!isValid)
                return;

            // Walidacja e-maila i telefonu
            ...
            try
            {
                ...
                ON DUPLICATE KEY UPDATE GuestID = LAST_INSERT_ID(GuestID);";
                    ...
                    int guestID;
                    using (var cmd = new MySqlCommand(guestInsertQuery, conn))
                    {
                ...
                VALUES (@GuestID, @RoomID, @CheckINDate, @CheckOUTDate, @TotalPrice, @ReservationCode, 'Potwierdzono');";
                    ...
                    using (var cmd = new MySqlCommand(bookingInsertQuery, conn))
                    {
